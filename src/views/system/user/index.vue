            ...
            <el-col :span="20" :xs="24">
                ...
                <el-row :gutter="10" class="mb8">
                    ...
                    <el-col :span="1.5" v-for="(v, i) in projectSlotButtonList.entrySlotButtonList" :key="i">
                        <el-button :type="v.type" plain :icon="v.icon" :loading="isloading" :size="v.size"
        ...
        <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="80px">
                <el-row>
                    ...
                    <el-col :span="12">
                        <el-form-item label="归属机构" prop="organizationId">
                            <treeselect v-model="form.organizationId" :options="deptOptions" :show-count="true"
                                placeholder="请选择归属机构" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                    ...
                    <el-col :span="24">
                        <el-form-item label="用户签名">
                            <el-upload class="sign-uploader" :action="signUploadUrl()" :show-file-list="false"
                                :on-success="handleSignSuccess">
                                <img v-if="signImageUrl" :src="signImageUrl" class="sign" />
                                <i v-else class="el-icon-plus sign-uploader-icon"></i>
                            </el-upload>
                        </el-form-item>
                    </el-col>
                </el-row>
...
import default_sign from "@/assets/images/default_sign.png";
...
export default {
    ...
    async created() {
         ...
         * @des 手动挂载
         */
        this.projectSlotButtonList = await this.getProjectSlotButtons('userButton')
        console.log('this.projectSlotButtonList =', this.projectSlotButtonList)
    },
    methods: {
        signUploadUrl() {
            return window.globalEnv.VUE_APP_BASE_API + "/system/user/profile/sign";
        },
        handleSignSuccess(res, file) {
            if (200 === res.code) {
                // 上传成功，赋值保存，用于保存修改
                this.signImageUrl = window.globalEnv.VUE_APP_BASE_API + res.imgUrl;
                this.form.sign = res.imgUrl;
            }
        },
        /** 查询用户列表 */
        getList() {
        ...
        handleUpdate(row) {
            ...
            getUser(userId).then(response => {
                ...
                this.title = "修改用户";
                this.form.password = "";
                if (null === response.data.sign) {
                    this.signImageUrl = this.defaultSign;
                } else {
                    this.signImageUrl = window.globalEnv.VUE_APP_BASE_API + response.data.sign;
                }
            });
        },
        /** 重置密码按钮操作 */
        handleResetPwd(row) {
            this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
                ...
                inputErrorMessage: "用户密码需字母和数字组合并且长度不小于8位"
            }).then(({ value }) => {
        ...
        handleExport() {
            ...
            this.$net('system/user/export', 'get').then(re => {
                window.location.href =
                    window.globalEnv.VUE_APP_BASE_API +
                    '/common/download?fileName=' + encodeURI(re.msg) + '&delete=' + false + '&Authorization=' + getToken() + '&MenuId=' + MenuId
            })
        },
        ...
        importTemplate() {
            ...
            this.$net('system/user/importTemplate', 'get').then(re => {
                window.location.href =
                    window.globalEnv.VUE_APP_BASE_API +
                    '/common/download?fileName=' + encodeURI(re.msg) + '&delete=' + false + '&Authorization=' + getToken() + '&MenuId=' + MenuId
            })
