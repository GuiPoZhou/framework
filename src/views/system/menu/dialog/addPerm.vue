<template>
  ...
  >
    <template slot="bologbody">
      <el-table :data="permsList" style="width: 100%">
        <el-table-column
          prop="menuName"
          align="center"
          label="权限名称"
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button
              type="primary"
              size="mini"
              @click="e_add(scope.row)"
              :disabled="submiLoading"
              >添加
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async e_getPerList() {
      ...
      let microRouterInfo = await axios.get(url, {
        headers: { Authorization: getToken() },
      });
      let microRouterList = JSON.parse(
        JSON.stringify(microRouterInfo.data.routerList)
      );
      console.log("microRouterList", microRouterList);
      ...
      if (microFuncRouterList.length != 0) {
        ...
        microFuncRouterInfo.permList.map((item) => {
          if (
            hasPerms.indexOf(
              `${microRouterInfo.data.moduleName}:${microFuncRouterInfo.name}:${item.symbolName}`
            ) == -1
          ) {
            let obj = {
              menuName: item.name,
