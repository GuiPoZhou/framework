    ...
    getList() {
      if (this.KevinJson.KevinWidget.manuallyEvents.getList) {
        new Function(
          "ctx",
          "_this",
          this.KevinJson.KevinWidget.manuallyEvents.getList
        )(this, this.$refs.KevinRender);
      } else {
        console.error("请配置表单手动执行脚本，并命名为 getList");
      }
    },
    async e_getWidgetInfo() {
      let re = await this.$net("/formLayout/v2/getFormLayoutConfig", "get", {
        id: this.widgetId,
      });
      try {
    ...
    runWidgetAutoEvents(autoEvents) {
      if (autoEvents) {
        for (let key in autoEvents) {
          new Function("ctx", "_this", autoEvents[key])(
            this,
            this.$refs.KevinRender
          );
        }
      }
      this.form = { ...this.form };
