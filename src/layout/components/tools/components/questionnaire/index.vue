<template>
    <div>
        <bo-dialog diaLogTitle="调查问卷" :diaLogShow="dialogVisible" diaLogWidth="30%" :isFullScreen="true" @close="e_close">
            <template slot="bologbody">
                <div class="questionnairearea">
                    ...
                    <el-form :model="ruleForm" ref="ruleForm" label-width="130px" class="demo-ruleForm"
                        label-position="top">
                        <div class="formarea">
                            <div class="fa-list" v-for="(v, i) in extendedData" :key="i">
                                    ...
                                    <div v-if="v.type == 'radio'">
                                        <el-radio-group v-model="v.value" :disabled="businessEnv=='detail'">
                                            <el-radio v-for="(radioInfo, radioIndex) in v.extendedDataList"
                                                :key="radioIndex" :label="radioInfo.value">{{ radioInfo.label }}</el-radio>
                                        </el-radio-group>
                                    </div>
                                    <div v-if="v.type == 'textarea'" >
                                        <el-input :disabled="businessEnv=='detail'" type="textarea" v-model="v.value" placeholder="请输入内容" autosize></el-input>
                                    </div>
                                </el-form-item>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        e_save() {
            this.$net('/v1/satisfactionSurvey/templateWrite', 'post',{ templateId: this.questionnaireInfo.id, extendedData: JSON.stringify(this.extendedData) }).then(re => {
                if (re.code = 200) {
