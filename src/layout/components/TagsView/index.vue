  ...
  >
    ...
    >
      ...
      >
        ...
        />
        <div v-if="isActive(tag)" class="line"></div>
      </router-link>
    </scroll-pane>
    <ul
      v-show="visible"
...
export default {
  ...
  watch: {
    $route() {
      this.getId(this.$store.state.permission.sidebarRouters);
      this.addTags();
  ...
  methods: {
    ...
    e_clickTag(tag) {
      ...
      localStorage.setItem("menuId", tag.menuId);
    },
    isActive(route) {
      if (route.path === this.$route.path) {
        localStorage.setItem("menuId", route.menuId);
      }
    ...
    closeSelectedTag(view) {
      this.doCloseSelectTag(view);
      // if (this.isActive(view)) {
      //   this.$confirm("确定关闭当前页面?").then(() => {
      //     this.doCloseSelectTag(view);
      //   });
      // } else {
      //   this.doCloseSelectTag(view);
...
.line {
  width: 100%;
  height: 2px;
  background-color: var(--my-navTopFountColor);
}
</style>
