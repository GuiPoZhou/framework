<template>
  <div class="im-container" v-if="visible">
    <div class="header">
      <el-avatar :size="30" :src="baseURL+info.avatar" />
      <span class="name">{{info.nickName}}/{{info.userName}}</span>
    ...
    <div class="main">
        ...
        <div class="chatList" ref="chatList">
          <div class="chatList-item" v-for="(item,index) in list" :key="index"
            ...
            <div class="chatList-user" v-else>
              <el-avatar :size="40" :src="baseURL+info.avatar" />
              <cite>{{info.nickName}}<i>{{item.dateTime}}</i></cite>
            </div>
            <div class="chatList-text">
              ...
              <el-image :src="baseURL+item.message.allUrl" class="chatList__msg--img"
                v-if="item.messageType=='image' && item.message.allUrl"
                :preview-src-list="[baseURL+item.message.allUrl]" :z-index="10000">
              </el-image>
              <audio class="chatList__msg--audio" controls :src="baseURL+item.message.path"
                v-if="item.messageType=='voice' && item.message.path"
                @click="handleEvent('voice',baseURL+item.message.path)"></audio>
        ...
        <div class="toolBox">
          <div class="toolBox-left">
            <el-popover placement="top-start" trigger="click" ref="popover" v-model="popoverVisible"
              ...
              <el-link icon="el-icon-picture-outline-round" :underline="false" title="发送表情"
                slot="reference">
              </el-link>
            </el-popover>
            <el-upload :show-file-list="false" :action="apiUpload" :limit="1"
              ...
              accept="image/*">
              <el-link icon="el-icon-picture" :underline="false" title="发送图片">
              </el-link>
            </el-upload>
          </div>
          <div class="toolBox-right">
            <el-link icon="el-icon-time" :underline="false" @click="openHistory">
              聊天记录
            </el-link>
          </div>
        </div>
      ...
      <div class="historyBox" v-if="showHistory">
        <el-input v-model="keyword" placeholder="搜索：请输入关键词" class="QMS-common-head-input" clearable
    ...
    <el-dialog :visible.sync="show" width="40%" :before-close="handleClose"
      class="QMS-dialog QMS-dialog_center" lock-scroll style="z-index:3000">
...
export default {
  ...
  data() {
    return {
      ...
      historyList: [],
      historyDefaultList: [],
      apiUpload: window.globalEnv.VUE_APP_BASE_API + '/common/upload',
      currentPage: 1,
      pageSize: 50,
      finish: false,
      ajaxing: false,
  ...
  methods: {
    ...
    getList(data) {
      ...
      for (let i = 0; i < data.list.length; i++) {
        ...
        list.push({
          ...
          dateTime: this.toDate(item.sendTime)
        })
      }
      if (!this.showHistory) {
        this.list = list
    ...
    sendList() {
      ...
      let messageList = {
        method: "MessageList",
        toUserId: String(this.info.userId),
        formUserId: String(this.userInfo.userId),
        token: this.$store.getters.token,
