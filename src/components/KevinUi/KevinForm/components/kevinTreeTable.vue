<template>
  <div class="HTable" style="margin-top:10px">
    <el-table ref="elTable" width="100%" v-bind="$attrs" :data="tableData" v-on="$listeners"
      :key="JSON.stringify(checkedCol)" @selection-change="handleSelectionChange">
...
export default {
  ...
  watch: {
    columns: {
      handler(newVal) {
        ...
        hotVal.forEach(item => {
          if (String(item.checked) == 'undefined') {
            item.checked = true
          }
        })
        this.col = hotVal.map(
          (item, index) =>
            (item = { ...item, index, checked: item.checked })
        );
  ...
  methods: {
    checkSelectable(row) {
      ...
      return new Function('ctx', '_this', 'row', this.$attrs.checkSelectable)(this.parentContext.ctx, this.parentContext._this, row)
    },
