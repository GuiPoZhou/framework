  ...
  <div>
    <kevindrawer :show="showDrawer" :title="title" direction="rtl" :size="size">
      <template slot="topbuttons">
        <el-button size="mini" type="primary" @click="e_doSave()"
          >保存</el-button
        >
      ...
      <div class="widgetEditArea">
        <component
          ...
          @close="close"
          @save="save"
          @selDBTable="e_selDbTable"
          @selLowCode="e_selLowCode"
        >
        </component>
      </div>
    </kevindrawer>
    ...
    <dbTables
      ref="dbTable"
      v-if="showDbTable"
      @close="showDbTable = false"
      @confirm="e_confirmDbInfo"
    />
    <lowcodeList
      ref="lowcodeList"
...
<script>
import lowcodeList from "./components/lowcodeList.vue";
import dbTables from "./components/dbTables.vue";
import menuPerm from "@/views/system/kevinFormManage/components/menuPerm.vue";
...
import timePickerEdit from "./widgetEditComponents/timePickerEdit.vue";
...
import equipmentChartEdit from "./widgetEditComponents/equipmentChartEdit.vue";
import kevinDialogEdit from "./widgetEditComponents/kevinDialogEdit.vue";
import kevinVNodeEdit from "./widgetEditComponents/kevinVNodeEdit.vue";
import kevinVerificationPlanModuleEdit from "./widgetEditComponents/kevinVerificationPlanModuleEdit.vue";
import wangEditorEdit from "./widgetEditComponents/wangEditorEdit.vue";
export default {
  components: {
    menuPerm,
    widgetTree,
    ...
    lowcodeList,
    kevinVNodeEdit,
    kevinVerificationPlanModuleEdit,
    wangEditorEdit,
  },
  props: {
    KevinWidget: Array,
  ...
  methods: {
    e_confirmLowCode(params) {
      this.showlowcodeList = false;
      this.$refs.component.confirmLowCode(params);
    },
    e_selLowCode() {
    ...
    e_confirmDbInfo(params) {
      this.showDbTable = false;
      this.$refs.component.setDbTable(params);
    },
    e_selDbTable() {
    ...
    e_confirmPerm(perm) {
      ...
      this.$refs.component.setPerm(perm);
    },
    e_selPerm() {
    ...
    e_showwidgetTree() {
      ...
      this.$nextTick(() => {
        this.$refs.widgetTree.init(this.KevinWidget);
      });
    },
    e_doSave() {
      this.$confirm("确定保存当前修改？").then(() => {
        this.$refs.component.e_save();
      });
    },
    e_openApiDoc() {
    ...
    renderSignleEdit(widgetInfo) {
      switch (widgetInfo.components) {
        ...
        case "el-date-picker":
          ...
          break;
        case "el-time-picker":
          this.conponentsName = "timePickerEdit";
          this.title = "时间组件编辑";
          this.size = "90%";
          break;
        case "el-radio":
        ...
        case "kevin_equipment_chart":
          this.conponentsName = "equipmentChartEdit";
          this.title = "设备状态图表";
          this.size = "70%";
          break;
        case "kevin-dialog":
          this.conponentsName = "kevinDialogEdit";
          this.title = "弹框组件编辑";
          this.size = "90%";
          break;
        case "kevin-v-node":
          this.conponentsName = "kevinVNodeEdit";
          this.title = "虚拟节点编辑";
          this.size = "100%";
          break;
        case "kevin-verificationPlan-module":
          this.conponentsName = "kevinVerificationPlanModuleEdit";
          this.title = "计量模板编辑";
          this.size = "60%";
          break;
        case "kevin-wang-editor":
          this.conponentsName = "wangEditorEdit";
          this.title = "富文本组件编辑";
          this.size = "60%";
           break;
        default:
    ...
    close() {
      this.$confirm("取消操作不会保存当前修改").then(() => {
