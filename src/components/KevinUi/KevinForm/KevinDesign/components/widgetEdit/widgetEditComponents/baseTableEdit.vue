    ...
    <widgetEditContainer>
      ...
      >
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="是否展示表头" prop="showHeader">
              <el-select
                v-model="editParams.showHeader"
                @change="e_changeHeader"
              >
          ...
          <el-col :span="12">
            <el-form-item label="开启合并行" prop="openMerge">
              <el-select v-model="editParams.openMerge">
                <el-option label="是" :value="true"></el-option>
                <el-option label="否" :value="false"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12" v-if="editParams.openMerge">
            <el-form-item
              label="合并内容"
              prop="mergeArr"
              :rules="[
            ...
            >
              <el-input
                v-model="editParams.mergeArr"
                placeholder="请输入要合并的prop,多个以英文逗号分隔"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="24">
            ...
            >
                    ...
                    <el-row>
                      ...
                      <el-col :span="6" v-if="scope.row.type == 'selection'">
                        <el-form-item label="是否保留勾选数据">
                          <el-select
                            size="mini"
                            v-model="scope.row.reserveSelection"
                          >
                            <el-option label="是" :value="true"></el-option>
                            <el-option label="否" :value="false"></el-option>
                          </el-select>
              ...
              <el-table-column prop="label" label="列名(label)" align="center">
                <template slot-scope="scope">
                  <el-input
                    :disabled="scope.row.type == 'expand'"
                    size="mini"
              ...
              <el-table-column
                ...
                align="center"
              ></el-table-column>
              <!-- <el-table-column prop="prop" label="自动处理超长内容" align="center">
                                <template slot-scope="scope">
              ...
              <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                  <el-button
                    v-if="scope.row.type != 'expand'"
                    type="text"
                    @click="e_deleteColumns(scope.$index)"
                    >删除</el-button
                  >
                </template>
              </el-table-column>
                ...
                <template slot-scope="scope">
                  <div class="perm_area">
                    ...
                    <el-button
                      class="p_a_button"
                      type="text"
                      @click="e_editActionButtonPerm(scope)"
                      >选择</el-button
                    >
                  </div>
                </template>
              </el-table-column>
              <el-table-column prop="title" label="禁用脚本" align="center">
                <template slot-scope="scope">
                  <el-button
                    type="text"
                    @click="e_editActionDisabledEvents(scope)"
                    >编辑</el-button
                  >
                </template>
              </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="单元格点击事件(cell-click)" prop="events.cellClick">
        ...
        <el-divider content-position="left">展开行属性</el-divider>
        <el-row>
          <el-col :span="12">
            <el-form-item label="是否开启展开行" prop="openExpand">
              <el-select
                size="mini"
                v-model="editParams.openExpand"
                @change="e_handleOpenExpand"
              >
                <el-option label="是" :value="true"></el-option>
                <el-option label="否" :value="false"></el-option>
              </el-select>
          ...
          <el-col :span="12" v-if="editParams.openExpand">
            <el-form-item label="展开行内容类型" prop="expandType">
              <el-select size="mini" v-model="editParams.expandType">
                <el-option label="普通表格" value="ELTable"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12" v-if="editParams.openExpand">
            <el-form-item>
              <el-button
                :disabled="!editParams.expandType"
                type="primary"
                size="small"
                @click="e_configExpand"
                >配置展开行内容</el-button
              >
            </el-form-item>
          </el-col>
...
import expandTable from "./components/tableExpand/expand_table.vue";
...
export default {
  ...
  methods: {
    e_confirmExpandinfo(params) {
      this.showComponent = false;
      this.editParams.expandOptions = params;
    },
    e_configExpand() {
      if (this.editParams.expandType == "ELTable") {
        this.conponentsName = "expandTable";
        this.showComponent = true;
        if (!this.editParams.expandOptions) {
          this.editParams.expandOptions = {
            tableDataKey: "childList",
            tableColumns: [],
            tableActions: [],
            tableSelect: {},
            events: {},
        ...
        this.$nextTick(() => {
          this.$refs.component.init(this.editParams.expandOptions);
        });
      }
    },
    e_handleOpenExpand() {
      if (this.editParams.openExpand) {
        let params = {
          type: "expand",
          fixed: "left",
          checked: true,
        ...
        this.editParams.tableColumns.unshift(params);
      } else {
        this.editParams.tableColumns = this.editParams.tableColumns.filter(
          (item) => {
    ...
    e_editActionDisabledEvents(scope) {
      this.editType = "tableActionDisabledEvents";
      this.editIndex = scope.$index;
      this.$refs.KevinEditors.changeEditor({
        value: scope.row.disabledEvents || "return false",
      });
    },
    ...
    handleEditorInput(code) {
      ...
      } else if (this.editType == "tableActionDisabledEvents") {
        this.editParams.tableActions[this.editIndex].disabledEvents =
          this.formatCode(code);
    ...
    e_save() {
      if (this.editParams.openMerge) {
        this.editParams.mergeArr = this.editParams.mergeArr.split(",");
        this.editParams.mergeObj = {};
        this.editParams.mergeArr.forEach((item) => {
          this.editParams.mergeObj[item] = [];
        });
      }
    ...
    init(widgetInfo) {
      ...
      if (this.editParams.openMerge) {
        this.editParams.mergeArr = this.editParams.mergeArr.join(",");
      }
      if (this.editParams.showHeader == undefined) {
        this.editParams.showHeader = true;
      }
      let _this = this;
      _this.$nextTick(() => {
