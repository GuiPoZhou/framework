    ...
    >
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item
            label="切换标签之前的钩子"
            prop="attributes.events.beforeLeaveEvents"
          >
            <el-input
              v-model="editParams.attributes.events.beforeLeaveEvents"
              readonly
            >
              <el-button
                slot="append"
                size="small"
                @click="e_editTabsbeforeLeaveEvents"
                >编辑</el-button
              >
            </el-input>
          </el-form-item>
          ...
          >
            ...
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  v-if="editParams.children.length > 1"
                  @click="e_deleteTabsChild(scope.$index)"
                  >删除</el-button
                >
              </template>
            </el-table-column>
  ...
  methods: {
    rowDropmainHomeButtons() {
      ...
      new Sortable(tbody, {
        ...
        //结束拖拽
        onEnd: function (obj) {
          const list = JSON.parse(JSON.stringify(that.editParams.children));
    ...
    e_editTabsbeforeLeaveEvents() {
      ...
      this.$refs.KevinEditors.changeEditor({
        value:
          this.editParams.attributes.events.beforeLeaveEvents || "return true",
      });
    },
    e_editTabsClickEvents() {
      ...
      this.$refs.KevinEditors.changeEditor({
        value:
          this.editParams.attributes.events.tabClick ||
          'console.log("选中事件")',
      });
    },
    handleEditorInput(code) {
      if (this.editType == "tabclick") {
        ...
        this.editParams.attributes.events.beforeLeaveEvents =
