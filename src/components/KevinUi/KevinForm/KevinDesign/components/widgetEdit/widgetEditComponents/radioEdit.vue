<template>
    <widgetEditContainer>
        <el-form slot="wecLeft" :model="editParams" label-position="top" ref="editParams" label-width="130px"
            class="demo-ruleForm">
            <el-divider content-position="left">单选框基本属性</el-divider>
            <el-row>
                <el-col :span="12">
                    <el-form-item label="Radio标题" prop="title"
                        :rules="[{ required: true, message: '请输入radio组件标题', trigger: 'blur' }]">
            ...
            <el-divider content-position="left">单选框事件配置</el-divider>
            <el-row>
                <el-col :span="12">
...
export default {
    ...
    methods: {
        e_editInput() {
            this.editType = 'inputEvents'
            this.$refs.KevinEditors.changeEditor({ value: this.editParams.events.input || "console.log('选择radio',value)" });
        },
        ...
        handleEditorInput(code) {
            if (this.editType == 'inputEvents') {
                this.$set(this.editParams.events, 'input', this.formatCode(code))
