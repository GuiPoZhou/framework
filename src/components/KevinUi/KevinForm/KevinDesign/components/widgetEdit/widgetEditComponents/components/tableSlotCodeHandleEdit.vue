        ...
        <div class="b_b_c_left">
          ...
          >
            <el-divider content-position="left"
              >脚本处理组件基础属性</el-divider
            >
            <el-row>
              <el-col :span="12">
                <el-form-item label="处理后展示组件" prop="showComponent">
                  <el-select v-model="slotParams.showComponent">
                    <el-option label="普通文本" value="Span"></el-option>
                    <el-option label="虚拟节点" value="VNode"></el-option>
            ...
            <el-divider content-position="left">脚本虚拟节点 Dom</el-divider>
            <el-row>
              <el-col :span="12">
                <el-form-item label="DOM脚本" prop="vNodeCode">
                  <el-input v-model="slotParams.vNodeCode" read only>
                    <el-button slot="append" @click="e_editVNodeCode"
                      >编辑</el-button
                    >
                  </el-input>
                </el-form-item>
              ...
              <el-col :span="12">
                <el-form-item label="返回值脚本" prop="events.handle">
                  <el-input v-model="slotParams.events.handle" read only>
                    <el-button slot="append" @click="e_editHandle"
                      >编辑</el-button
                    >
                  </el-input>
                </el-form-item>
...
export default {
  ...
  methods: {
    e_editVNodeCode() {
      this.editType = "vNodeCode";
      this.$refs.KevinEditors.changeEditor({
        value:
          this.slotParams.vNodeCode ||
          "return createElement( 'div', { style: { width: '100%', maxHeight: '30px', }, }, '虚拟节点' )",
      });
    },
    e_editHandle() {
      this.editType = "editHandle";
      this.$refs.KevinEditors.changeEditor({
        value: this.slotParams.events.handle || "return scope.row",
      });
    },
    handleEditorInput(code) {
      if (this.editType == "editHandle") {
        this.$set(this.slotParams.events, "handle", this.formatCode(code));
      } else if (this.editType == "vNodeCode") {
        this.$set(this.slotParams, "vNodeCode", this.formatCode(code));
      }
    },
    formatCode(code) {
      // 去除开头和结尾的空白字符
