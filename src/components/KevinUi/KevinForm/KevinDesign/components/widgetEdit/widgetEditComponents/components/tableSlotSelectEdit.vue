          ...
          >
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item
                  label="获取远程数据脚本"
                  prop="dataSource.optionsGetEvents"
                >
                  <el-input
                    v-model="slotParams.dataSource.optionsGetEvents"
                    readonly
                  >
                    <el-button
                      slot="append"
              ...
              <el-col :span="12">
                <el-form-item
                  label="远程脚本执行方式"
                  prop="dataSource.executionMethod"
                >
              ...
              <el-col :span="12" style="margin-bottom: 0.3rem">
                <el-button
                  ...
                  >远程脚本测试</el-button
                >
                <el-button size="mini" type="warning" @click="e_addOptions"
                  >手动新增</el-button
                >
              </el-col>
              <el-col :span="24">
                <el-table
                  :data="slotParams.dataSource.list"
                  style="width: 100%"
                  border
                >
                  <el-table-column
                  ...
                  <el-table-column
                    label="操作(删除只对手动添加方式生效)"
                    align="center"
                  >
                    <template slot-scope="scope">
                      <el-button
                        type="text"
                        @click="e_deleteSource(scope.$index)"
                        >删除</el-button
                      >
                    </template>
                  </el-table-column>
              ...
              <!-- <el-col :span="12">
                <el-form-item label="回车事件" prop="events.enter">
                  <el-input v-model="slotParams.events.enter" readonly>
                    <el-button slot="append" @click="e_editEnter"
                      >编辑</el-button
                    >
                  </el-input>
                </el-form-item>
              ...
              <el-col :span="12" v-if="slotParams.clearable">
                <el-form-item label="Clear事件" prop="events.clear">
                  <el-input v-model="slotParams.events.clear" readonly>
                    <el-button slot="append" @click="e_editClear"
                      >编辑</el-button
                    >
                  </el-input>
                </el-form-item>
  ...
  methods: {
    ...
    e_addOptions() {
      ...
      if (this.slotParams.dataSource.optionsGetEvents) {
        this.$confirm(
          "检测到你已配置远程数据获取，如果通过手动方式新增，远程获取数据源脚本将被清空，是否继续？"
        ).then(() => {
    ...
    e_tastOptionsGetEvents() {
      ...
      new Function(
        ...
        "columnsInfo",
        this.slotParams.dataSource.optionsGetEvents
      )(window.KevinContext, { vModelActionOptions: this.slotParams });
    },
    e_editSelectServerEvents() {
      ...
      this.$refs.KevinEditors.changeEditor({
        value:
          this.slotParams.dataSource.optionsGetEvents ||
          "let params = { label: '测试', value: 0, }; columnsInfo.vModelActionOptions.dataSource.list = [params]; ",
      });
    },
    e_editEnter() {
      this.editType = "editEnter";
      this.$refs.KevinEditors.changeEditor({
        value: this.slotParams.events.enter,
      });
    },
    e_editClear() {
      this.editType = "editClear";
      this.$refs.KevinEditors.changeEditor({
        value: this.slotParams.events.clear,
      });
    },
    e_editBlur() {
