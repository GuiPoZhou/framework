<template>
  <div style="overflow: hidden">
    <KevinDrawer
      ...
      :stepList="stepList"
      :stepIndex="stepIndex"
      :title="KevinJson.KevinWidget.title"
    >
      <template slot="topbuttons">
        <el-button size="mini" :disabled="stepIndex == 0" @click="e_goPrevious"
          >上一步</el-button
        >
        <el-button
          size="mini"
        ...
        <el-button
          ...
          >保存</el-button
        >
        <el-button size="mini" @click="e_close">取消</el-button>
        <el-button
          size="mini"
          type="warning"
      ...
      <widgetFormEdit
        ref="widgetFormEdit"
        v-if="stepIndex == 0"
        @close="showwidgetFormEdit = false"
            ...
            >
              <el-row>
                <kevin-render
                  ...
                  :context="context"
                  :form="form"
                  :Widget="KevinJson.KevinWidget.children"
                  :draggableOpen="true"
                  @editTabs="e_editTabs"
                  @editWidget="e_editWidget"
                  @saveWidgetNodeEdit="e_saveWidgetNodeEdit"
                />
              </el-row>
            </el-form>
          </div>
    ...
    <widgetNodeEdit
      ref="widgetNodeEdit"
      :KevinWidget="KevinJson.KevinWidget.children"
      v-if="showwidgetNodeEdit"
      @close="showwidgetNodeEdit = false"
      @save="e_saveWidgetNodeEdit"
    />
    <widgetTree
      ref="widgetTree"
      v-if="showwidgetTree"
      @close="showwidgetTree = false"
    />
    <!-- 表单预览 -->
    <showForm
      ref="showForm"
      v-if="showshowForm"
      @close="showshowForm = false"
    />
    <!-- api文档 -->
...
export default {
  ...
  methods: {
    getList() {},
    e_demo() {
      console.log(this.getKeyList(this.KevinJson.KevinWidget.children));
    },
    getKeyList(tree) {
      for (const node of tree) {
        if (node.vModelType && node.vModelType == "ext" && node.tableName) {
          let obj = {
            label: node.title,
            keyName: node.vModel,
        ...
        if (node.components == "el-table") {
          for (const tc of node.tableColumns) {
            if (
              tc.type != "selection" &&
              tc.type != "action" &&
              tc.type != "index" &&
              ...
              if (tc.vModelType && tc.vModelType == "ext" && tc.tableName) {
                let tcParams = {
                  label: tc.label,
                  keyName: tc.prop,
                  tableName: tc.tableName,
                };
                this.keyList.push(tcParams);
              }
            }
          }
          if (node.expandOptions && node.expandOptions.tableColumns) {
            for (const tc of node.expandOptions.tableColumns) {
              if (
                tc.type != "selection" &&
                tc.type != "action" &&
                tc.type != "index" &&
        ...
        if (node.components == "el-table-tree") {
          for (const tc of node.tableColumns) {
            if (
              tc.type != "selection" &&
              tc.type != "action" &&
    ...
    e_showForm() {
      ...
      this.$nextTick(() => {
        this.$refs.showForm.init(JSON.parse(JSON.stringify(this.KevinJson)));
      });
    },
    e_saveWidgetNodeEdit(widgetInfo) {
      this.showwidgetNodeEdit = false;
      this.modifyNodeByWIDGETID(
        this.KevinJson.KevinWidget.children,
        widgetInfo.WIDGETID,
        widgetInfo
      );
      ...
      this.$refs.KevinRenders.getWidgetServerData(
        this.KevinJson.KevinWidget.children
      );
    },
    ...
    e_saveWidgetBasicInfo(info) {
      ...
      this.KevinJson.KevinWidget.fullScreen = info.fullScreen;
    ...
    e_editWidgetForm() {
      let editParams = {
        ...
        width: this.KevinJson.KevinWidget.width || 60,
        diaLogFooterButton: this.KevinJson.KevinWidget.diaLogFooterButton,
        fullScreen: this.KevinJson.KevinWidget.fullScreen,
      };
      this.$nextTick(() => {
    ...
    modifyNodeByWIDGETID(nodeList, targetWIDGETID, newData) {
      for (let i = 0; i < nodeList.length; i++) {
        ...
        if (node.WIDGETID === targetWIDGETID) {
          ...
          // 递归遍历子节点
          if (
            this.modifyNodeByWIDGETID(node.children, targetWIDGETID, newData)
          ) {
    ...
    e_save() {
      this.$confirm("确定完成表单编制进行保存").then(() => {
        ...
        this.getKeyList(this.KevinJson.KevinWidget.children);
        ...
        let params = {
          ...
          dynamicColumnConfigList: this.keyList,
        };
        this.$net("/formLayout/v2/saveOrUpdateFormLayoutConfig", "post", params).then(
          (re) => {
            if (re.code == 200) {
